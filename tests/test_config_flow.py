...
async def test_user_form_invalid_auth(hass, mock_api):
    ...
    result = {"type": data_entry_flow.RESULT_TYPE_FORM, "errors": {"base": "invalid_auth"}}
...
async def test_user_form_unique_id(hass, mock_api):
    ...
    MockConfigEntry(domain=DOMAIN, unique_id="existing-username").add_to_hass(hass)

    result = {"type": data_entry_flow.RESULT_TYPE_ABORT, "reason": "already_configured"}
    hass.config_entries.flow.async_configure.return_value = result
